# This script performs internal validation on RISE outer folds.

################### Internal validation on RISE outer fold
library(data.table)
library(rsample)
library(ggplot2)
library(tidyverse)
...
intermediate_directory <- './3_intermediate/external_validation/updates/rise/nested_model'  # directory to store external validation results for sanbs
if (!dir.exists(intermediate_directory)) {
  dir.create(intermediate_directory)
}
...
## load rise data ----
outer_fold_test_dt_path <- "./3_intermediate/outer_fold_test_data/"
...
if (predict_outcome == "predict_hgb"){
  if (train_version == "hgb_only"){
    ...
    load_model_and_predict(version = "hgb_only_predict_hgb",
                           res_dir='./3_intermediate/external_validation/updates/rise/nested_model/',
...
# Rscript --vanilla /home/wanjinli/iron-ml-ext-val/2_scripts/07_rise_internal_validation.R "predict_hgb" "hgb_only" "outer_fold_1"
