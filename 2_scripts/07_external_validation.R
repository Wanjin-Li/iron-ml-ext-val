# This script performs external validation on SA and US datasets.
...
# (UNUSED) External validation on outer fold of RISE dataset  ----
...
## load SANBS data ----
dat_paths <- c("./3_intermediate/private/hgb_ferr_sanbs.csv",
               "./3_intermediate/private/hgb_only_sanbs.csv")
...
for (dat_path in dat_paths) {
  ...
  if (dat_path == "./3_intermediate/private/hgb_ferr_sanbs.csv") {
    hf_pred_h_factor <- dt.md.hgb
    hf_pred_h_oh <- dt.OH.hgb
    hf_pred_f_factor <- dt.md.ferr
    hf_pred_f_oh <- dt.OH.ferr
  } else if (dat_path == "./3_intermediate/private/hgb_only_sanbs.csv") {
