...
intermediate_directory <- './external_validation_sanquin'  # directory to store external validation results
...
load_model_and_predict <- function(version, res_dir) {
  ...
  model_paths <- list.files(path="./trained_models", pattern=version_pattern, full.names = TRUE) 
  ...
  for (path in model_paths) {  # for each model in ensemble
    ...
    file_name <- gsub("^.*/", "", path)  # get last part of path: "hgb_ferr_predict_hgb_5XGB1RF_RF.208.rds"
    file_name <- paste0(gsub(".rds", "", file_name), ".csv")  # remove ".rds" and add ".csv" -> # "hgb_ferr_predict_hgb_5XGB1RF_RF.208.csv"
    print(file_name)
    fwrite(results, paste0(res_dir, file_name)) # "./external_validation_sanquin/hgb_ferr_predict_hgb_5XGB1RF_RF.208.csv"
  }
}
...
load_model_and_predict(version = "hgb_ferr_predict_hgb",
                       res_dir='./external_validation_sanquin/')
...
#TODO: send external_validation_sanquin folder back to chen-yang.su@mail.mcgill.ca
