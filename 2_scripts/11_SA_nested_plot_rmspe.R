# This script plots eFigure 7: sensitivity analysis results on single donation 
# for RISE (internal validation), SA, US, NL.
...
for (fold in outer_folds){
  for (version in versions) {
    ...
    rise_model_res_paths <- list.files(path="./3_intermediate/sensitivity_analysis/results/rise/nested_model", pattern=paste0(pattern_spec, collapse = ".+"), full.names = TRUE) 
...
for (version in versions) {
  print(version)
  sanbs_model_res_paths <- list.files(path="./3_intermediate/sensitivity_analysis/results/sanbs", pattern=version, full.names = TRUE) 
...
for (version in versions) {
  print(version)
  vitalant_model_res_paths <- list.files(path="./3_intermediate/sensitivity_analysis/results/vitalant", pattern=version, full.names = TRUE) 
...
for (version in versions) {
  print(version)
  sanquin_model_res_path <- list.files(path="./3_intermediate/sensitivity_analysis/results/sanquin", pattern=version, full.names = TRUE) 
...
cohort <- c("RISE", "Vitalant", "SANBS", "Sanquin") # To add NL results back
...
# plot the results with individual values for nested CV


# Plot eFIGURE 7 RMSPE ----
...
fname_svg <- paste0("./4_output/updates/figs/", "SA_fig3.svg") 
fname_png <- paste0("./4_output/updates/figs/", "SA_fig3.png")
