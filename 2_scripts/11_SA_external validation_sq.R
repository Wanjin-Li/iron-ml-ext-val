...
# on external cohorts with single donation per donor (NL only).
...
load_model_and_predict <- function(version, res_dir) {
  ...
  model_paths <- list.files(path="./3_intermediate/trained_models/updates", pattern=version_pattern, full.names = TRUE) # replace with the correct path to extract the trained models
...
# NOTE: To get the single donation data from 11_SA_single_donor_data_prep_sq.R 

dat_paths <- c("./path/private/single_donation_data/hgb_ferr_sanquin.csv", # replace with the correct path
               "./path/private/single_donation_data/hgb_only_sanquin.csv")
...
for (dat_path in dat_paths) {
  ...
  if (dat_path == "./path/private/single_donation_data/hgb_ferr_sanquin.csv") { 
    hf_pred_h_factor <- dt.md.hgb
    hf_pred_h_oh <- dt.OH.hgb
    hf_pred_f_factor <- dt.md.ferr
    hf_pred_f_oh <- dt.OH.ferr
  } else if (dat_path == "./path/private/single_donation_data/hgb_only_sanquin.csv") {
...
# NOTE: replace with the correct path for res_dir to save the sensitivity analysis result but not overwrite the initial external validation result

# Ensemble
load_model_and_predict(version = "hgb_ferr_predict_hgb",
                       res_dir='./3_intermediate/sensitivity_analysis/results/sanquin/') 
